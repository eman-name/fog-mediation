#!/usr/bin/env python2
#
# Implemented and modified from OpenFace project
# 
...
# This function does import images, face detection and neural network forward pass
...
# The main training function, I simply use LinearSVM by default, other options can be changed in handler.py
def train(args):
...
# classification, it will produce the confidence score with chosen model for each image.
def classify(args, multiple=False):
    ...
    imgs = list(iterImgs(args.imgs))

    for img in imgs:
        print("\n=== {} ===".format(img.path))
        reps = getRep(img.path, multiple)
        if len(reps) > 1:
...
if __name__ == '__main__':
    ...
    inferParser.add_argument('imgs', type=str, help="Input image.")
    ...
    if args.mode == 'classify' and args.classifierModel.endswith(".t7"):
        raise Exception("""
